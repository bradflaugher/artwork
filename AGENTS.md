...
- **Poem count:** 31 poems (Batch 1: 15, Batch 2: 8, Batch 3: 8). Bump the count in `README.md` "Now reading" when adding a poem; add one epigraph spotlight line for the new piece.
- **Batch fit:** Geography, landscape, and celestial themes (mountains, trees, stars, wind, thunder, rain, clouds, weather, rooms, ocean) default to **Batch 1 — surreal / liminal** unless the piece is primarily a form, inventory, or procedure — then use Batch 2 or 3. Bureaucratic surrealism in the poem body still belongs in Batch 1 when the strangeness is image-first, not form-first.
...
- **Branch sync (loop 1):** Complete. `mm/loop-1` fast-forwarded to `origin/mm/loop-2` (e9775d3); geography trilogy peers (trees, mountain, ocean) on disk.
...
- **Ocean mission (loop 2):** Complete. `the-tide-submits-a-correction.md` — Batch 1, bureaucratic surrealism (inverted tides, memos in bottles, counter-memos, back-dated wave inadmissible). Geography trilogy complete (trees → mountain → ocean).
- **Ocean ship (loop 4):** Shipped on `mm/loop-4`. Skip guard: `grep -riE 'ocean|sea|tide' poems/` and `grep 'the-tide-submits-a-correction' poems/index.md README.md`. No second ocean piece unless the human requests it.
